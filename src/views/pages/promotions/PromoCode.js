...
const PromoCode = () => {
    ...
    const loadData = (count, timer) => {
        if (user && token) {
            ...
            let url = BASE_URL + `promocodes/all/${count}?name=${searchQuery}`
    ...
    const handleActivate = () => {
        ...
        if (user && token) {
            axios
                .patch(BASE_URL + 'promocode/update/status/' + promotionId, data, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                })
    ...
    const deletePromotion = (id) => {
        ...
        if (user && token) {
            axios
                .patch(BASE_URL + 'promocode/delete/' + id, data, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                })
    ...
    return (
        <CContainer>
            <Link to={`/promotions/add/promocode`} className="picker-link">
            ...
            {loading ?  <div className="d-flex justify-content-center"><CSpinner style={{marginTop:"15%"}}/></div> : <CTable>
                <CTableHead>
                    <CTableRow>
                        ...
                        <CTableHeaderCell scope="col">Discount Type</CTableHeaderCell>
                        <CTableHeaderCell scope="col">Discount</CTableHeaderCell>
                        <CTableHeaderCell scope="col">Type</CTableHeaderCell>
                        <CTableHeaderCell scope="col">Max Use</CTableHeaderCell>
                        <CTableHeaderCell scope="col">Min Total</CTableHeaderCell>
                        <CTableHeaderCell scope="col">Max Per User</CTableHeaderCell>
                ...
                <CTableBody>
                    {promotionData.length === 0 ? (
                                    <CTableRow>
                                        <CTableDataCell colSpan="15" style={{ textAlign: 'center', backgroundColor: "white" }}>
                    ...
                    promotionData.map((item, index) => (
                        <CTableRow key={index}>
                            ...
                            <CTableDataCell>{new Date(item.start).toLocaleDateString()}</CTableDataCell>
                            <CTableDataCell>{new Date(item.end).toLocaleDateString()}</CTableDataCell>
                            <CTableDataCell>{item.discountType === 'percentage' ? 'Percentage' : "Amount"}</CTableDataCell>
                            <CTableDataCell>{item.discount}</CTableDataCell>
                            <CTableDataCell>{item.type === 'delivery_fee' ? "Delivery Fee" : "Sub Total"}</CTableDataCell>
                            <CTableDataCell>{item.maxUse}</CTableDataCell>
                            <CTableDataCell>{item.minTotal}</CTableDataCell>
                            <CTableDataCell>{item.maxUsePerUser}</CTableDataCell>
                            <CTableDataCell style={{maxWidth : '100px'}}>{item.description}</CTableDataCell>
