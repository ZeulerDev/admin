...
const Drivers = () => {
  ...
  const loadDriversData = () => {
    if (user && token) {
      setLoading(true)
      let url = `https://15.160.211.157/assistant/riders/:skip?city=${paramCity}&group=${paramGroup}&chain=${paramChainId}`
  ...
  const handleSubmit = () => {
    if(name && surname && email && phone && iban && cityEdit && vat && address){
      ...
      if(user,token){
          if(user && token){
              axios
                .patch('https://15.160.211.157/assistant/rider/update/'+id, formData, {
                  headers: {
                    Authorization: `Bearer ${token}`,
                  },
                })
